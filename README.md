...
7. User can change the Emulator speed via the second drop-down, the default speed is 8 cycles/second.  
8. When the `Reset` button is click, the Emulator memory, register and speed to set to their original values. The instructions and register displays are cleared, but the test out put remain, since the test program acts on a different CPU object.
9. The `Step Forward` and `Step Backward` buttons are not working yet, we will add them in release 3. 
...
11. For testing, just click the `Start Test` button, new test cases has been added since the last release. We will migrate to Jest in release 3. 
