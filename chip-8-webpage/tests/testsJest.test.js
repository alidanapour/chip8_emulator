...
var functions = require('../scripts/testsJest');
...
test( 'Tool editor clear: test a random value to be 0x00', ()=>{
    expect(functions.toolEditorClear()).toBe(0x00);
});

test( 'Tool editor up shift', ()=>{
    expectedArray = [0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x10, 0x11, 0x12, 0x13, 0x14, 0x00];
    expect(functions.toolMoveSpriteUp()).toEqual(expectedArray);
});

test( 'Tool editor down shift', ()=>{
    expectedArray = [0x14, 0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x10, 0x11, 0x12, 0x13];
    expect(functions.toolMoveSpriteDown()).toEqual(expectedArray);
});

test( 'Tool editor left shift', ()=>{
    expectedArray = [0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40, 0x40];
    expect(functions.toolMoveSpriteLeft()).toEqual(expectedArray);
});

test( 'Tool editor right shift', ()=>{
    expectedArray = [0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10, 0x10];
    expect(functions.toolMoveSpriteRight()).toEqual(expectedArray);
});
